$(document).ready(function() {
    ...
    if ($('#highcharts-customers').length > 0) {
        const $chart = $('#highcharts-customers');
        const csrf   = $chart.data('csrf');
        const data   = getHighchartsData($chart.data('url'), csrf);

        Highcharts.chart('highcharts-customers', {
            series: data,
        });
    }

    if ($('#highcharts-products-sold').length > 0) {
        const $chart = $('#highcharts-products-sold');
        const csrf   = $chart.data('csrf');
        const data   = getHighchartsData($chart.data('url'), csrf);

        Highcharts.chart('highcharts-products-sold', {
            series: data,
        });
    }
});
